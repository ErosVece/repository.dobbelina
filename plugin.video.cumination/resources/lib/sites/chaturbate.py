...
def onlineFav(url):
    ...
    chaturbate_url = f'https://chaturbate.com/affiliates/api/onlinerooms/?format=json&wm={random.choice(wmArray)}'
    ...
    for (name, url, image) in result:
        if model := [
            ...
            if item["username"] == name.split('[COLOR')[0].strip()
        ]:
            image = model[0]["image_url"]
            current_show = ''
            if "current_show" in model[0]:
                if model[0]["current_show"] != "public":
                    current_show = f'[COLOR blue] {model[0]["current_show"]}[/COLOR]'
...
def login():
    ...
    if f'title="Username Dropdown">{username}<' not in response:
        utils.notify('Chaturbate', 'Login failed please check your username and password')


@site.register()
def Unfollow(id):
    url = f'https://chaturbate.com/follow/unfollow/{id}/'
    ...
    if '"following": false' in response:
        utils.notify('Chaturbate', f'NOT FOLLOWING [COLOR hotpink]{id}[/COLOR]')
        utils.refresh()


@site.register()
def Follow(id):
    url = f'https://chaturbate.com/follow/follow/{id}/'
    html = utils._getHtml(url, site.url)
    if '<h1>Chaturbate Login</h1>' in html:
        login()
        html = utils._getHtml(url, site.url)
    ...
    if '"following": true' in response:
        utils.notify('Chaturbate', f'FOLLOWING [COLOR hotpink]{id}[/COLOR]')
        utils.refresh()
